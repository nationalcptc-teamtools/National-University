...
```shell-session
anasec@htb[/htb]$ echo -n dXNlcj1odGItc3RkbnQ7cm9sZT11c2Vy | base64 -d
...
```shell-session
anasec@htb[/htb]$ echo -n 'user=htb-stdnt;role=admin' | base64
...
```shell-session
anasec@htb[/htb]$ echo -n 'user=htb-stdnt;role=admin' | xxd -p

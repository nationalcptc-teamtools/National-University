...
```shell-session
anasec@htb[/htb]$ wget https://raw.githubusercontent.com/danielmiessler/SecLists/master/Miscellaneous/Web/content-type.txt
anasec@htb[/htb]$ cat content-type.txt | grep 'image/' > image-content-types.txt
```
...
```shell-session
anasec@htb[/htb]$ echo "this is a text file" > text.jpg 
anasec@htb[/htb]$ file text.jpg 
text.jpg: ASCII text
```
...
```shell-session
anasec@htb[/htb]$ echo "GIF8" > text.jpg 
anasec@htb[/htb]$file text.jpg

...
```shell-session
anasec@htb[/htb]$ echo -n 1 | md5sum
...
```shell-session
anasec@htb[/htb]$ echo -n 1 | base64 -w 0 | md5sum
...
```shell-session
anasec@htb[/htb]$ for i in {1..10}; do echo -n $i | base64 -w 0 | md5sum | tr -d ' -'; done
...
```shell-session
anasec@htb[/htb]$ bash ./exploit.sh
anasec@htb[/htb]$ ls -1

...
```shell-session
anasec@htb[/htb]$ proxychains curl http://SERVER_IP:PORT
...
```shell-session
anasec@htb[/htb]$ nmap -h | grep -i prox
...
```shell-session
anasec@htb[/htb]$ nmap --proxies http://127.0.0.1:8080 SERVER_IP -pPORT -Pn -sC

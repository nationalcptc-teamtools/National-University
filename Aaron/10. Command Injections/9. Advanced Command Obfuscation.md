...
```shell-session
anasec@htb[/htb]$ echo 'whoami' | rev
imaohw
```
...
```shell-session
anasec@htb[/htb]$ echo -n 'cat /etc/passwd | grep 33' | base64
...
```shell-session
anasec@htb[/htb]$ bash<<<$(base64 -d<<<Y2F0IC9ldGMvcGFzc3dkIHwgZ3JlcCAzMw==)
...
```shell-session
anasec@htb[/htb]$ echo -n whoami | iconv -f utf-8 -t utf-16le | base64

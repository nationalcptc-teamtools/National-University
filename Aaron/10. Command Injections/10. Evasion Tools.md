...
```shell-session
anasec@htb[/htb]$ git clone https://github.com/Bashfuscator/Bashfuscator
anasec@htb[/htb]$ cd Bashfuscator
anasec@htb[/htb]$ pip3 install setuptools==65
anasec@htb[/htb]$ python3 setup.py install --user
```
...
```shell-session
anasec@htb[/htb]$ cd ./bashfuscator/bin/
anasec@htb[/htb]$ ./bashfuscator -h
...
```shell-session
anasec@htb[/htb]$ ./bashfuscator -c 'cat /etc/passwd'
...
```shell-session
anasec@htb[/htb]$ ./bashfuscator -c 'cat /etc/passwd' -s 1 -t 1 --no-mangling --layers 1
...
```shell-session
anasec@htb[/htb]$ bash -c 'eval "$(W0=(w \  t e c p s a \/ d);for Ll in 4 7 2 1 8 3 2 4 8 5 7 6 6 0 9;{ printf %s "${W0[$Ll]}";};)"'
